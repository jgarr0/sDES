...
def multicore_decrypt(dir, errorbytes, length, rule, IV, start, end):
    try:
...
def adderr(_KEY, _PLAINTEXT, _IV, _NUM_ERRORS):
    ...
    # check if results directory exists
    dir2 = "./results/{}_{}_brute_results".format(plaintext, _NUM_ERRORS)
    ...
    with mp.Pool() as pool:
        try:
            pool.starmap(multicore_decrypt, [(dir2, errortext_bytes, ciphertextlength ,RULE, IV, lowerbounds[i], upperbounds[i]) for i in range(0, numcore)])
        except KeyboardInterrupt:
